...
\author{Martin Morgan\footnote{<Martin.Morgan```RoswellPark.org>}}
...
manner. This document applies to the following `*Param()`:

\begin{itemize}
\item `SerialParam()`: sequential evaluation in a single *R* process.
\item `SnowParam()`: parallel evaluation in multiple independent
  *R* processes.
\item `MulticoreParam())`: parallel evaluation in *R*
...
\begin{itemize}
\item `bplapply()`: `lapply()`-like application of a
  user-supplied function `FUN` to a vector or list of elements
  `X`.
\item `bpiterate()`: apply a user-supplied function `FUN` to
...
\begin{itemize}
\item `bptry()` and the `BPREDO=` argument, for
...
For reproducible calculation, use the `RNGseed=` argument in any
of the `*Param()` constructors.
...
performing the evaluation) and `tasks` (how elements of `X`
are distributed between workers). Results are invariant to these
parameters. This is illustrated with `SnowParam()`, but applies
also to `MulticoreParam()`.

```{r different-workers-tasks}
...
Subsequent sections illustrate results with `SerialParam()`, but
identical results are obtained with `SnowParam()` and
`MulticoreParam()`.

## Use with `bpiterate()`

`bpiterate()` allows parallel processing of a 'stream' of data as
...
with a simple example -- iterating through a vector `x = 1:3`

```{r bpiterate-ITER_FUN_FACTORY}
...
`ITER\_FUN\_FACTORY()` is used to create a function that, on each invocation,
returns the next task (here, an element of `x`; in a real
example, perhaps 100000 records from a BAM file). When there are no
more tasks, the function returns `NULL`
...
computations as `bplapply()` so the results, including the
random number streams used by each task in `bpiterate()`, are
the same
...
`bptry()` in conjunction with the `BPREDO=` argument to
`bplapply()` or `bpiterate()` allows for graceful recovery
from errors. Here a buggy `FUN1()` produces an error for the
second element. `bptry()` allows evaluation to continue for
other elements of `X`, despite the error. This is shown in the
result.
...
`FUN2()` illustrates the flexibility of `bptry()` by fixing
the bug when `i == 2`, but also generating incorrect results if
...
When `RNGseed=` is not used, an internal stream (not accessible
to the user) is used and \BiocParallel{} does NOT increment the global stream.

```{r not-RNGseed-and-set-seed}
set.seed(100)
value <- runif(1)
...
`bplapply()` or `bpiterate()` in the 'stopped'
state. Internally, `bplapply()` and `bpiterate()` invoke
`bpstart()` to establish the computational environment (e.g.,
starting workers for `SnowParam()`). `bpstart()` can be
...
This allows a second call to `bplapply` to represent a
continuation of a random number computation -- the second call to
`bplapply()` results in different random number streams for each
element of `X`.

```{r bpstart-continuation}
param <- bpstart(SerialParam(RNGseed = 100))
result16 <- bplapply(1:3, runif, BPPARAM = param)
...
`lapply()`, even with the same random number seed. This is
...
`?RNGkind` and `?parallel::clusterSetRNGStream` for
...
`bpstart()` creates a new stream from the L'Ecuyer-CMRG
generator. Each element in `bplapply()` or `bpiterate()`
creates a new substream. Each application of `FUN` is therefore
...
Within the user-supplied `FUN` of `bplapply()` or
`bpiterate()`, it is a mistake to use `RNGkind()` to set a
different random number generator, or to use `set.seed()`. This
