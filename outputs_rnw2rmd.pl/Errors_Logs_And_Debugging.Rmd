...
logging available through `SnowParam` and `MulticoreParam` objects.
...
`SnowParam()` or `BatchjobsParaam()`) so should not be
...
\BiocParallel{} functions support a `BPREDO` (re-do) argument for
...
When `log = TRUE` the \CRANpkg{futile.logger} package is loaded on
...
By default `log` is FALSE and `threshold` is {\it INFO}.
...
different levels of severity. A message created with `flog.debug` will
...
The debug messages reveal the problem occurs when `x' is `numeric()`.
The index for `sapply` is along `v' which in this case has length 0.
This forces `i' to take values of `1' and `0' giving an output of length 2 for
the second element (i.e., `NA` and `numeric(0)`).
...
``fun2'' can be fixed by using `seq\_along(v)` to create the index
