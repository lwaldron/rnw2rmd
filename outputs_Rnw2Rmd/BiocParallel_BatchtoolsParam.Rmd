...
\begin{itemize}
  ...
  - {\Rcode{workers}} The number of workers used by the job.
  ...
  - {\Rcode{registryargs}}
    ...
    \begin{description}
      ...
      - {\Rcode{file.dir}} Path where all files of the registry are
      ...
      - {\Rcode{work.dir}} Working directory for R process for
        running jobs.

      - {\Rcode{packages}} Packages that will be loaded on each node.

      - {\Rcode{namespaces}} Namespaces that will be loaded on each
        node.

      - {\Rcode{source}} Files that are sourced before executing a
        job.

      - {\Rcode{load}} Files that are loaded before executing a job.
...
```
  ...
  - {\Rcode{resources}} A named list of key-value pairs to be
    subsituted into the template file; see
    \Rcode{?batchtools::submitJobs}.

  - {\Rcode{template}} The template argument is unique to the
  ...
  - {\Rcode{log}} The log option is logical, TRUE/FALSE. If it is
  ...
  - {\Rcode{logdir}} Path to the logs. It is given only if
    \Rcode{log=TRUE}.

  - {\Rcode{resultdir}} Path to the directory is given when the job
