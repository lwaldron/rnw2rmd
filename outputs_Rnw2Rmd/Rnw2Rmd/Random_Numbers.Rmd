...
\begin{itemize}
-  \Rcode{SerialParam()}: sequential evaluation in a single *R* process.
-  \Rcode{SnowParam()}: parallel evaluation in multiple independent
  *R* processes.
-  \Rcode{MulticoreParam())}: parallel evaluation in *R*
...
\begin{itemize}
-  \Rcode{bplapply()}: \Rcode{lapply()}-like application of a
  user-supplied function \Rcode{FUN} to a vector or list of elements
  \Rcode{X}.
-  \Rcode{bpiterate()}: apply a user-supplied function \Rcode{FUN} to
...
\begin{itemize}
-  \Rcode{bptry()} and the \Rcode{BPREDO=} argument, for
